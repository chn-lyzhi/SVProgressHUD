...
//加
@property (nonatomic, strong) UIImage * errorImageCMM;
@property (nonatomic, strong) UIImage * successImageCMM;
@property (nonatomic, strong) UIImage * successTeacherImage;
@property (nonatomic, strong) UIImage * errorTeacherImage;
...
+ (SVProgressHUD*)sharedView{
    ...
    dispatch_once(&once, ^{ sharedView = [[SVProgressHUD alloc] initWithFrame:[[[UIApplication sharedApplication] delegate] window].bounds]; });
#else
...
+ (void)showInfoWithStatus:(NSString*)status{
    [self showImage:[self sharedView].infoImage status:status backImage:nil];
}
...
+ (void)showImage:(UIImage*)image status:(NSString*)status backImage:(UIImage *)backImage{
    NSTimeInterval displayInterval = [self displayDurationForString:status];
    [[self sharedView] showImage:image status:status duration:displayInterval backImage:backImage];
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    if((self = [super initWithFrame:frame])){
        ...
        //加的
        NSURL *urlCMM = [bundle URLForResource:@"CMMProgressHUD" withExtension:@"bundle"];
        NSBundle *imageBundleCMM = [NSBundle bundleWithURL:urlCMM];
        
        UIImage * successImageCMM = [UIImage imageWithContentsOfFile:[imageBundleCMM pathForResource:@"投递成功-弹框" ofType:@"png"]];
        UIImage * errorImageCMM = [UIImage imageWithContentsOfFile:[imageBundleCMM pathForResource:@"消息提醒-弹框" ofType:@"png"]];
        _successImageCMM = successImageCMM;
        _errorImageCMM= errorImageCMM;
        
        _successTeacherImage = [UIImage imageNamed:@"导师弹框"];
        _errorTeacherImage = [UIImage imageNamed:@"teacher小风车-弹框-0"];
...
- (void)updateHUDFrame{
    CGFloat hudWidth = self.hudView.image ? self.hudView.image.size.width : 100;
    //加
    CGFloat hudHeight = self.hudView.image ? self.hudView.image.size.height : 100;
...
#pragma clang diagnostic pop
    ...
    self.hudView.bounds = CGRectMake(0.0f, 0.0f, MAX(self.minimumSize.width, hudWidth), hudHeight);
    ...
    if(string) {
        ...
        //        CGFloat centerHeight = hudHeight > 100 ? 36.0f :
        ...
        CGFloat centerHeight = hudHeight > 100 ? 190.f : CGRectGetHeight(self.hudView.bounds)/2;
        CGPoint center = CGPointMake((CGRectGetWidth(self.hudView.bounds)/2), centerHeight);
...
- (void)showImage:(UIImage*)image status:(NSString*)string duration:(NSTimeInterval)duration backImage:(UIImage *)backImage {
...
- (UIImageView*)hudView{
    ...
    //    _hudView.image = [UIColor colorWithPatternImage:successImage];
    //    _hudView.image = successImage;
